...
Implements the reverse-engineered protocol documented in ``PROTOCOL.md`` (in
this folder). The 27-byte full state frame and the 16-byte Quiet/Powerful short
frames are both produced here as protocol-agnostic microsecond timings: a flat
``list[int]`` where a positive value is a pulse (carrier on) and a negative
value is a space (carrier off), matching the Home Assistant ``infrared``
platform convention.
"""
