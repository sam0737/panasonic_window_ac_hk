...
2. Search for **"Panasonic"** or **"Window"** (full title: _Panasonic
   Window-Type Air Conditioner (Hong Kong / Macau)_).
3. In the form, enter a **Name** and pick the **Infrared emitter** for that room.
4. Repeat for each A/C.

If the wizard aborts with _"No infrared emitters were found"_, set up an IR
...
- **0.5 °C** steps: the protocol encodes temperature as `°C × 2` (byte 14), so
  half degrees are representable. Whether your specific unit _acts_ on the
